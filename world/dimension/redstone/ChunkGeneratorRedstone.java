...
public class ChunkGeneratorRedstone implements IChunkGenerator
{
	//Block that replaces End Stone
	protected static final IBlockState MAIN_BLOCK = Blocks.REDSTONE_BLOCK.getDefaultState();
    ...
    public ChunkGeneratorRedstone(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
    {
        this.world = world;
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        {
            abyte[i] = (byte)Biome.getIdForBiome(BiomeInit.REDSTONE_BIOME);
        }
    ...
    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
    {
        ...
        for (int i = -12; i <= 12; ++i)
        {
            for (int j = -12; j <= 12; ++j)
            {
                ...
                if (k * k + l * l > 4096L && ChunkGeneratorRedstone.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
                {
